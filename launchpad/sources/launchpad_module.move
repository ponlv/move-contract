module shoshinlaunchpad::launchpad_module {
        ...
        * 
        */
        public entry fun make_buy_mint_nft<T: store + key>(whitelist_container: &mut WhitelistContainer, launchpad: &mut Launchpad, coin: Coin<SUI>, round_id: ID, clock: &Clock, ctx: &mut TxContext) {
                // check owner
                assert!(launchpad.owner_address != tx_context::sender(ctx), EWasOwned);
        ...
        * 
        */
        public entry fun make_buy_public_mint_nft<T: store + key>(whitelist_container: &mut WhitelistContainer, launchpad: &mut Launchpad, coin: Coin<SUI>, round_id: ID, clock: &Clock, ctx: &mut TxContext) {
                // check owner
                assert!(launchpad.owner_address != tx_context::sender(ctx), EWasOwned);
                assert!(launchpad.is_deposit == false, ENotHavePermistion);
                // check total supply
