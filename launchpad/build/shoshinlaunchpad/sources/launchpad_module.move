module shoshinlaunchpad::launchpad_module {
        ...
        */
        ...
        public entry fun create_launchpad(admin:&mut Admin, name : String, ctx: &mut TxContext) {
                // check admin
                let sender = tx_context::sender(ctx);
        ...
        */
        ...
        public entry fun make_buy_nft<T: store + key>(launchpad: &mut Launchpad, coin: Coin<SUI>, project_id: ID, round_id: ID, current_time: u64, ctx: &mut TxContext) {
                // borrow mut from launchpad
                let project = ofield::borrow_mut<ID, Project<T>>(&mut launchpad.id, project_id);  
                ...
                // transfer
                let price_balance:Balance<SUI> = balance::split(coin::balance_mut(&mut coin), current_price);
